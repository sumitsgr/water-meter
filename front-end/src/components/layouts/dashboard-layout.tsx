...
const Logo = () => {
  return (
    <Link className="flex items-center text-white" to={paths.home.getHref()}>
      ...
      {/* <span className="text-sm font-semibold text-white">Water Meter</span> */}
    </Link>
  );
};
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    checkAccess({ allowedRoles: [ROLES.SUPER_ADMIN || ROLES.ADMIN] }) && {
      name: 'Dashboard',
      to: paths.app.dashboard.getHref(),
      icon: Home,
    },
    // { name: 'Dashboard', to: paths.app.dashboard.getHref(), icon: Home },
    { name: 'Devices', to: paths.app.devices.getHref(), icon: Gauge },
    { name: 'Alarms', to: paths.app.alarms.getHref(), icon: Bell },
